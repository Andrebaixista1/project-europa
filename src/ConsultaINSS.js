...
const ConsultaINSS = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.data.disbursementBankAccount?.bank) {
        try {
          const bankResponse = await axios.get(
            `https://brasilapi.com.br/api/banks/v1/${response.data.disbursementBankAccount.bank}`
          );
          setDisbursementBankName(`${bankResponse.data.code} - ${bankResponse.data.fullName}`);
        } catch {
      ...
      await saveToSupabase(response.data);
  ...
  const handleCopy = () => {
    ...
    const dataToCopy = Object.entries({
      ...
      "Tipo de Crédito":
        dados.creditType === "checking_account" ? "Conta Corrente" : "Cartão Magnético",
      ...
      "Banco de Desembolso": disbursementBankName || "-",
            ...
            <tbody>
              {Object.entries({
                ...
                if (key === "Tipo de Bloqueio") {
                  ...
                  textClass =
                    tipoBloqueio === "nenhum" ? "text-success fw-bold" : "text-danger fw-bold";
                }
                return (
                  <tr key={key}>
                    <th>{key}</th>
